...
${printTable(actions)}\n`;
  ...
  and(otherPredicate, otherErrorMessage) {
    this.destroy();
    return new ActionMatcher(allPass([this.predicate, otherPredicate]), `${this.errorMessage} and ${otherErrorMessage}`, this.store, this.timeout);
  }
  ...
  matching(objectOrPredicate) {
    if (typeof objectOrPredicate === "function") {
      ...
      return this.and(equals(objectOrPredicate), `equal to ${trySerialize(objectOrPredicate)}`);
    }
  }
...
class NotActionMatcher extends ActionMatcher {
  ...
  and(otherPredicate, otherErrorMessage) {
    this.destroy();
    return new NotActionMatcher(allPass([this.predicate, otherPredicate]), `${this.errorMessage} and ${otherErrorMessage}`, this.store, this.timeout);
  }
